...
object AddVertices {
  ...
  private def noneExist(v1: VertexStructDb, v2: VertexStructDb, p1: List[KeyValue[String]], p2: List[KeyValue[String]], pE: List[KeyValue[String]]): Unit = {
    v1.addVertex(p1, label, gremlinConnector.graph, gremlinConnector.b)
    v2.addVertex(p2, label, gremlinConnector.graph, gremlinConnector.b)
    createEdge(v1, v2, pE)
  }
  ...
  private def oneExist(v1: VertexStructDb, v2: VertexStructDb, p1: List[KeyValue[String]], p2: List[KeyValue[String]], pE: List[KeyValue[String]]): Unit = {
    if(v1.exist) {
      v2.addVertex(p2, "aLabel", gremlinConnector.graph, gremlinConnector.b)
      createEdge(v1, v2, pE)
    } else {
      v1.addVertex(p1, "aLabel", gremlinConnector.graph, gremlinConnector.b)
      createEdge(v1, v2, pE)
    }
  }
  ...
  private def createEdge(v1: VertexStructDb, v2: VertexStructDb, pE: List[KeyValue[String]]): Unit = {
    implicit val scalaGraph: ScalaGraph = gremlinConnector.graph
    val edge = v1.vertex --- "alabel" --> v2.vertex.element
    for(keyV <- pE) {
      gremlinConnector.g.E(edge).property(keyV)
    }
  }

  private def areVertexLinked(v1: VertexStructDb, v2: VertexStructDb): Boolean = {
    val oneWay = gremlinConnector.g.V(v1.vertex.id).outE().as("e").inV.has(ID, v2.vertex.id.toString).select("e").traversal
    val otherWay = gremlinConnector.g.V(v1.vertex.id).outE().as("e").inV.has(ID, v2.vertex.id.toString).select("e").traversal
